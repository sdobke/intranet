...
async function pnRegisterSW() {
	navigator.serviceWorker.register('PNServiceWorker.js')
		.then((swReg) => {
			...
			document.getElementById('err').innerHTML = 'Registracion exitosa';
		}).catch((error) => {
			// registration failed
			console.log('Registration failed with ' + error);
			document.getElementById('err').innerHTML = 'Error al intentar registrar: '+error;
		});
}
			...
			reg.unregister()
				.then(function (bOK) {
					if (bOK) {
						...
						document.getElementById('err').innerHTML = 'Baja a la suscripcion exitosa';
					} else {
						console.log('unregister service worker failed.');
						document.getElementById('err').innerHTML = 'Error intentando dar de baja la suscripcion';
					}
				});
		});
}
