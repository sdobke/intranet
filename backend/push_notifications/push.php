...
if (isset($_GET['idnota']) && is_numeric($_GET['idnota']) && $_GET['idnota'] > 0 && isset($_GET['tabnom']) && $_GET['tabnom'] != '' && isset($_GET['tipo']) && is_numeric($_GET['tipo']) && $_GET['tipo'] > 0) {
  include("../../cnfg/config.php");
  include("../../inc/funciones.php");
  $id = $_GET['idnota'];
  $tipo = $_GET['tipo'];
  $nombretab = $_GET['tabnom'];
  /**
  ...
  echo 'Cantidad de suscriptores: ' . $oDP->count() . '<br/><br/>' . PHP_EOL;
  if (!$oDP->init()) {
    echo $oDP->getError();
    exit();
  }
  ...
  if ($con == 1) {
    ...
    $copete = txtcod($noticia['copete']);
    include("../inc/img.php");
    $foto = '../../cliente/img/logo.png';
    $foto_ppal = imagenPpal($id, $tipo,4);
    if(is_array($foto_ppal)){
      //echo '<br>Es array';
    ...
    $oPayload = new PNPayload(txtcod($noticia['titulo']), $copete, $foto);
    $oPayload->setTag('news', true);
    $oPayload->setURL('https://bercovich.intranet.com.ar/nota.php?id='.$id.'&tipo='.$tipo);
    ...
    if (!$oServer->push()) {
      ...
      $aLog = $oServer->getLog();
      echo '<h2>Resumen:</h2>' . PHP_EOL;
      $summary = $oServer->getSummary();
      echo 'Total:&nbsp;' . $summary['total'] . '<br/>' . PHP_EOL;
      echo 'Enviados:&nbsp;' . $summary['pushed'] . '<br/>' . PHP_EOL;
      echo 'Fallaron:&nbsp;' . $summary['failed'] . '<br/>' . PHP_EOL;
      echo 'Expiraron:&nbsp;' . $summary['expired'] . '<br/>' . PHP_EOL;
      echo 'Eliminados:&nbsp;' . $summary['removed'] . '<br/>' . PHP_EOL;
