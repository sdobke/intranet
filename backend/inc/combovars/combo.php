...
$display.= '<label class="control-label" for="'.$var_combo.'">'.txtcod($titulo_combo).'</label>
		<div class="controls">';
// SELECT
...
$cbotab = $taborden;
if($idsel == 1){
	$display.='<input '.$taborden.' class="span2" id="id_'.$var_combo.'" name="id_'.$var_combo.'" type="text" value="'.$valor.'" onChange="cambiaSelect(this.value, this.id)" />';
	$cambiarid = 'onChange="cambiaInput(this.value, this.id)"';
	$spancod = 10;
	$cbotab = '';
}
$display.= '<select name="'.$var_combo.'" class="span'.$spancod.'" id="'.$var_combo.'" '.$cambiarid.$cbotab.'>';
...
while($row_combo = mysqli_fetch_array($res_combo)){
	...
	$display.= '<option value="'.$row_combo[$codid].'" '.$es_activo.' >'.txtcod($row_combo[$tit_combo]).'</option>';
}
$display.= '</select>
	</div> <!--cierra controls-->
';