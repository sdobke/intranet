...
Date: 20080910
*/
//$images_path = "/fotos/";  //replace this with the path where your images are stored, relative to the directory this script will run from
...
if(!imagecopyresampled($dest_image, $source_image, 0, 0, $left, $top, $dest_width, $dest_height, max($width, $max_width), max($height, $max_height))){
	die('{"error":"No se puede hacer el recorte con las coordenadas enviadas."}');
}
...
copy($source_file,$destino);
unlink($source_file);
$imagen = new SimpleImage();

// Imagen home no destacada
// Home

$destino_base = explode("dest",$_POST['destino'], -1);

$imagen->load($destino);
$limitew = config('destacadaw');
$limiteh = config('destacadah');
$imagen->centrar($limitew, $limiteh);
$destino_img = explode("dest",$_POST['destino'], -1);
$destino_img = $dir_ppal = end($destino_base).'dest.jpg';
$imagen->save($destino_img);
$destino_img = $dir_ppal = end($destino_base).'home.jpg';
$imagen->save($destino_img);
// Nota
$imagen->load($destino);
$limitew = config('notaw');
$limiteh = config('notah');
$imagen->centrar($limitew, $limiteh);
$destino_img = explode("dest",$_POST['destino'], -1);
$destino_img = $dir_ppal = end($destino_base).'nota.jpg';
$imagen->save($destino_img);
// Seccion
$imagen->load($destino);
$limitew = config('fotosecw');
$limiteh = config('fotosech');
$imagen->centrar($limitew, $limiteh);
$destino_img = explode("dest",$_POST['destino'], -1);
$destino_img = $dir_ppal = end($destino_base).'sec.jpg';
$imagen->save($destino_img);
// Thumb
$imagen->load($destino);
$limitew = config('thumb');
$limiteh = config('thumb');
$imagen->centrar($limitew, $limiteh);
$destino_img = explode("dest",$_POST['destino'], -1);
$destino_img = $dir_ppal = end($destino_base).'thumb.jpg';
$imagen->save($destino_img);
?>
<!DOCTYPE html>
