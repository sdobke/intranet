...
//echo '<br>backend: '.$backend;
//echo '<br>rest: '.$usarest;
//echo '<br>Tipoemp: '.$_SESSION['tipoemp'];
if (
	(!isset($backend) || (isset($backend) && $backend != 1))
	&&
	(isset($usarest) && $usarest == 1
		&&
		(!isset($_SESSION['backend'])) // Si está paralelamente logueado como admin de backend
	)
) {
	//echo 'do it';
	$where_restriccion = " AND (il.tipo = " . $tipo . ") ".$sql_restric." ";
	$inner_restriccion = "INNER JOIN " . $_SESSION['prefijo'] . "link AS il ON inov.id = il.item";
}else{
	//echo 'dont';
	//$inner_restriccion = "INNER JOIN " . $_SESSION['prefijo'] . "link AS il ON inov.id = il.item";
}
//echo '<br>Inner: '.$inner_restriccion.' WHERE: '.$where_restriccion;
$where_combos = '';
foreach ($_POST as $key => $value) { // Levanta todos los POST
	...
	if ($conta_bus > 0) { // Si existe la tabla
		if ($value > 0) { // Si el valor es mayor a cero
			...
			if($nom_post_combo == 'area'){
				$where_combos .= " AND " . $nom_post_combo . ".id = " . $value;
			}else{
...
switch ($tipo) {
	case 34: // Políticas
		$campos .= ' , inov.tipoarc AS tipoarc, inov.empresa AS empresa, inov.link AS link, inov.peso AS peso, inov.tipodoc AS tipodoc';
		//$inner_join = 'INNER JOIN emp_cargos AS c ON c.cod = inov.cargo';
		break;
	case 3: // Documentos Útiles
		$campos .= ' , inov.tipoarc AS tipoarc,inov.empresa AS empresa, inov.link AS link, inov.peso AS peso, inov.tipodoc AS tipodoc, inov.url AS url, inov.area AS area';
		//$inner_join = 'LEFT JOIN ' . $_SESSION["prefijo"] . 'areas AS area ON inov.area = area.id';
		break;
	case 7: // novedades
	...
	case 11: // Encuestas
		...
		break;
	case 4: // Empleados
		$campos .= ' , inov.apellido AS apellido, inov.empresa AS empresa, inov.interno AS interno, inov.fechanac AS fechanac, inov.email AS email ';
		$inner_join = ' INNER JOIN ' . $_SESSION["prefijo"] . 'empleados_areas AS iea ON iea.empleado = inov.id INNER JOIN ' . $_SESSION["prefijo"] . 'areas AS area ON area.id = iea.area ';
		break;
}
