...
function fechaInserta($fecha,$repetir,$frec,$fecha_hasta){
	
	$f_inicio = explode('-',$fecha);
	
	/* if ($repetir == 1 or $repetir == 2 or $repetir == 3)
		$f_final = date("Y-m-d", mktime(0, 0, 0, $f_inicio[1], ($f_inicio[2]+$frec), $f_inicio[0]));
		else
		$f_final = date("Y-m-d", mktime(0, 0, 0, ($f_inicio[1]+$frec), $f_inicio[2], $f_inicio[0]));*/
		
		$f_final = date("Y-m-d", mktime(0, 0, 0, $f_inicio[1], ($f_inicio[2]+$frec), $f_inicio[0]));
	
	$dia = strtolower(date("D", strtotime($f_final)));
	switch ($dia){
		case 'sat':
			$f_final = date('Y-m-d', strtotime($f_final .' -1 day'));
			break;
		case 'sun':
			$f_final = date('Y-m-d', strtotime($f_final .' -2 day'));
			break; 
	} 
...
function Salas1 ($hora,$horafin,$sala,$fecha){
	$qarch = 'reservas_funciones.php / Función: Salas1';
	$tmp = '';
...
function Salascontrola ($hora,$horafin,$sala,$fecha){
	$qarch = 'reservas_funciones.php / función: Salascontrola';
	$tmp = '';
	$sql_horario = "SELECT horario, horarioh FROM intranet_reservas_horarios WHERE horario >= '".$hora."' AND horarioh <= '".$horafin."' order by 1";
	$res_horario = fullQuery($sql_horario,$qarch);
	while($row_horario = mysqli_fetch_array($res_horario)){
...
function Salas00 ($hora,$sala,$fecha,$cantidad,$duracion,$usacap,$repetir,$fecha_hasta){
	$qarch = 'reservas_funciones.php / Función: Salas00';
	/* echo 'Hora '.$hora.'<br>';
	echo 'Sala '.$sala.'<br>';
	echo 'Fecha '.$fecha.'<br>';
	...
	if (!empty($row_salas['id'])){ // Si está reservada
		...
		if($row_salas['empleado'] == $_SESSION['usrfrontend']){ // Si la reserva la realizó el mismo usuario
			$tmp = '<td><div class="tip" data-tip="Reservada por '.$resemple.'">
						<form method="post" name="'.str_replace(array(":","-"),'',$hora.$sala.$fecha).'">
							...
							<input type="hidden" name="repeti" value="'.$repetir.'" />
		...
		$sql_serie = "SELECT repetidor FROM intranet_reservas WHERE hini <= '".$hora."' and hfin >= '".$hora."' and sala=".$sala." and fecha= '".$fecha."'";
		$res_serie = fullQuery($sql_serie,$qarch);
		$row_serie = mysqli_fetch_array($res_serie);	
		
		if (!empty($row_serie[0])) {
		
		/*
				$tmp = $tmp. '<div class="tip" data-tip="Reservada por '.$resemple.'">
						<form method="post" name="'.str_replace(array(":","-"),'',$hora.$sala.$fecha).'">
							<input type="hidden" name="borrar" value="'.$row_salas['id'].'" />
							<input type="hidden" name="opcion" value="S" />
							<input type="hidden" name="repeti" value="'.$repetir.'" />
							<input type="hidden" name="fecha_hasta" value="'.$fecha_hasta.'" />
							<input type="hidden" name="fechainicio" value="'.$fecha.'" />
							<input type="hidden" name="idserie" value="'.$row_serie[0].'" />
							<button type="submit" name="submit" value="Cancelar Reserva">
							<span class="icon icon35"></span><span class="labeled">Cancelar Serie</span></button>
						</form>';		
		}*/
		
		$nomform = str_replace(array(":","-"),'',$hora.$sala.$fecha);
		$tmp = '<td><div class="tip" data-tip="Reservada por '.$resemple.'">
						<form method="post" id ="'.$nomform.'" name="'.$nomform.'" onClick="tipoCancela(this);">
							<input type="hidden" id="'.$nomform.'borrar" name="borrar" value="'.$row_salas['id'].'" />
							<input type="hidden" id="'.$nomform.'opcion" name="opcion" value="S" />
							<input type="hidden" id="'.$nomform.'repeti" name="repeti" value="'.$repetir.'" />
							<input type="hidden" id="'.$nomform.'fecha_hasta" name="fecha_hasta" value="'.$fecha_hasta.'" />
							<input type="hidden" id="'.$nomform.'fechainicio" name="fechainicio" value="'.$fecha.'" />
							<input type="hidden" id="'.$nomform.'idserie" name="idserie" value="'.$row_serie[0].'" />
							<button type="button">
								<span class="icon icon35"></span><span class="labeled">Cancelar</span>
							</button>
						</form>';		
		}
...
function Salas30 ($hora,$sala,$fecha,$cantidad,$duracion,$usacap,$repetir,$fecha_hasta){
	$qarch = 'reservas_funciones.php / Función: Salas30';
	/*  echo 'Hora '.$hora.'<br>';
	echo 'Sala '.$sala.'<br>';
	echo 'Fecha '.$fecha.'<br>';
	echo 'Cantidad '.$cantidad.'<br>';
...
function LOVSalas ($numsala, $cantidad, $idsala){
	$qarch = 'reservas_funciones.php / Función: LOVSalas';
...
function LOVCantidad ($numsala, $cantidad){
	$qarch = 'reservas_funciones.php / Función: LOVCantidad';
	$sql_a = "SELECT  id,concat(nombre,' - (Capacidad Max: ',capacidad,' personas)'), capacidad
				FROM intranet_reservas_salas";
	if (!empty($numsala))
		$sql_a = $sql_a.' WHERE id NOT IN ('.$numsala.')';
...
function Salasugerida ($numsala, $cantidad){
	$qarch = 'reservas_funciones.php / Función: Salasugerida';
	$sql_a = "SELECT  id,concat(nombre,' - (Capacidad Max: ',capacidad,' personas)'), capacidad
				FROM intranet_reservas_salas";
