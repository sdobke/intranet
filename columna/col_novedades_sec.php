...
if ($titicon != '') {
	...
	$titicon = '<i class="fas fa-' . $titicon . '"></i>';
...
?>
	...
	<div id="secc_<?php echo $colsec; ?>" class="card">
		<div class="card-body novedades_sec">
				<h4><?PHP echo $nombre_sec; ?><span class="icono"><?php echo $titicon; ?></span></h4>
			<?php
			while ($col_row_rest = mysqli_fetch_array($col_res_rest)) {
			?>

				<div class="datos row card-text">
					<?PHP
					$col2 = 'col';
					$col_limtx = $col_orlimtx;
					//echo '<br>Col: '.$col_limtx;
					$col_base_id     = $col_row_rest['id'];
					$col_base_tit      = '<a class="title" href="nota.php?tipo=' . $tipo_col . '&id=' . $col_base_id . '">' . txtcod($col_row_rest['titulo']) . '</a>';
					if ($col_usafecha > 0) {
					...
					<div class="<?php echo $col2; ?>">
						...
						<?PHP if ($col_usafecha > 0) { ?>
							...
							<small class="fecha"><?PHP echo $col_base_fectx . fechaDet($col_base_fec, 'corto'); ?></small><br />

						<?PHP } ?>

						<?PHP echo $col_base_tit; ?>
						<?PHP if ($col_usatexto == 1) {
							$col_limtx = $col_limtx - (strlen($col_base_tit));
							echo '<p class="card-text"><small class="text-body-secondary">' . cortarTexto(txtcod($col_row_rest['texto']), $col_limtx) . '</small></p>';
			...
			?>
					...
					<a class="btn btn-outline-dark btn-sm more icon-link icon-link-hover" href="seccion.php?tipo=<?PHP echo $tipo_col; ?>&sec=<?php echo $tipo_secc_nov; ?>">
						Ver m&aacute;s <i class="bi bi-chevron-right"></i>	</a>
