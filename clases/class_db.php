...
class MySQL
{
	...
	private $db_dbname  = "citrica_db"; // database name
	...
	public function GetColumnComments($table) {
		$this->ResetError();
		$records = fullQuery("SHOW FULL COLUMNS FROM " . $table);
		if (! $records) {
			$this->SetError();
			return false;
		} else {
	...
	public function GetColumnCount($table = "") {
		...
		if (empty($table)) {
			...
			$records = fullQuery("SELECT * FROM " . $table . " LIMIT 1");
			if (! $records) {
				$this->SetError();
				$result = false;
			} else {
	...
	public function GetColumnDataType($column, $table = "") {
		...
		if (empty($table)) {
			...
			$result = fullQuery("SELECT " . $column . " FROM " . $table . " LIMIT 1");
   ...
   /**
	...
	public function GetColumnLength($column, $table = "") {
		...
		if (empty($table)) {
			...
			$records = fullQuery("SELECT " . $column . " FROM " . $table . " LIMIT 1");
			if (! $records) {
				$this->SetError();
				return false;
			}
   ...
   /**
	...
	public function GetColumnNames($table = "") {
		...
		if (empty($table)) {
			...
			$result = fullQuery("SHOW COLUMNS FROM " . $table);
			if (! $result) {
				$this->SetError();
				$columns = false;
			} else {
